...
TEST(Player, deleteCard) {
    Player p1(4, "user", 0);
    p1.deleteCard(0);
    ASSERT_EQ(3, p1.cards[0]);
    ASSERT_EQ(5, p1.cards[1]);
    ASSERT_EQ(8, p1.cards[2]);
    ASSERT_EQ(13, p1.cards[3]);
}

TEST(Player, printCards) {
    Player p1(4, "user", 0);
    //stdout redirect to a buffer
    std::stringstream buffer;
    std::streambuf * old = std::cout.rdbuf(buffer.rdbuf());
    p1.printCards();
    std::string text = buffer.str();
    std::cout.rdbuf(old);
    ASSERT_EQ("[user chess pieces]: [2, 3, 5, 8, 13]\n", text);
}

TEST(Player, constrctor_1) {
    Player p1(4, "user", 0);
    vector<int> tmp {2,3,5,8,13};
    bool a = p1.cards==tmp;
    ASSERT_EQ(true, a);
...
TEST(Board, checkBoard) {
    ...
    b1.whoscard[0][2] = 1;
    b1.checkBoard();
    ASSERT_EQ(0, b1.b[0][0]);
    ASSERT_EQ(-1, b1.whoscard[0][0]);
    ASSERT_EQ(0, b1.b[0][1]);
    ASSERT_EQ(-1, b1.whoscard[0][1]);
    ASSERT_EQ(2, b1.b[0][2]);
    ASSERT_EQ(1, b1.whoscard[0][2]);
}

TEST(Board, checkWinner_1) {
    Board b1(6);
    Player p0(6,"user1", 0);
    Player p1(6,"user2", 1);
    b1.b[0][0] = 2;
    ...
    ASSERT_EQ(p0.id, b1.checkWinner(p0,p1,0));
}

TEST(Board, checkWinner_2) {
    Board b1(6);
    Player p0(6,"user1", 0);
    Player p1(6,"user2", 1);
    b1.b[0][0] = 2;
    ...
    ASSERT_EQ(p1.id, b1.checkWinner(p0,p1,0));
}

TEST(Board, checkWinner_3) {
    Board b1(6);
    Player p0(6,"user1", 0);
    Player p1(6,"user2", 1);
    b1.b[0][0] = 2;
...
TEST(Board, checkWinner_5) {
    ...
    b1.whoscard[2][3] = p1.id;
    b1.whoscard[3][1] = p1.id;
    ASSERT_EQ(-1, b1.checkWinner(p0,p1,0));
}
...
TEST(Board, doaction) {
    ...
    a.index = 0;
    b1.doaction(a);
    ASSERT_EQ(2, b1.b[1][3]);
    ASSERT_EQ(0, b1.whoscard[1][3]);
}

TEST(Board, randomMove) {
    Board b1(4);
    Player p1(4, "user", 1);
    b1.randomMove(p1);
    ...
    ASSERT_EQ(col_1, col_2);
    ASSERT_EQ(b1.whoscard[row_1][col_1], p1.id);
}

TEST(Board, legalAction_1) {
    Board b1(4);
    action a;
    a.row = 1;
    a.col = 3;
    ...
    a.id = 0;
    a.index = 0;
    ASSERT_EQ(true, b1.legalAction(a));
}

TEST(Board, legalAction_2) {
    Board b1(4);
    b1.whoscard[1][3] = 2;
    action a;
    a.row = 1;
    ...
    a.id = 0;
    a.index = 0;
    ASSERT_EQ(false, b1.legalAction(a));
}

TEST(Board, constructor) {
    Board b1(4);
    ASSERT_EQ(4, b1.b.size());
    ASSERT_EQ(4, b1.b[0].size());
    ASSERT_EQ(4, b1.whoscard.size());
    ASSERT_EQ(4, b1.whoscard[0].size());
}
...
TEST(MCTS, search) {
    ...
    ASSERT_NE(-1, a.index);
    ASSERT_NE(-1, a.card);
    ASSERT_EQ(0, b1.b[a.row][a.col]);
    ASSERT_EQ(0, b1.whoscard[a.row][a.col]);
}

TEST(MCTS, selection_1) {
    MCTS m;
    node root;
    Board b1(4);
    Player p1(4, "user1", 1);
...
TEST(MCTS, selection_2) {
    ...
    node *ptr = m.selection(&root, b1, p1, p2);
    ASSERT_EQ(ptr, &root.children[0]);
}

TEST(MCTS, expansion_1) {
    MCTS m;
...
TEST(MCTS, expansion_5) {
    ...
    root->act.id = p1.id;
    node *tmp = m.expansion(root, b1,p1,p2);
    ASSERT_NE(-1, tmp->act.row);
    ASSERT_NE(-1, tmp->act.col);
    ASSERT_NE(-1, tmp->act.id);
    ASSERT_NE(-1, tmp->act.index);
    ASSERT_NE(-1, tmp->act.card);
}

TEST(MCTS, simulation_1) {
    MCTS m;
    node *root = (node*) malloc(1 * sizeof(node));
    Board b1(4);
    Player p1(4, "user1", 1);
    ...
    ASSERT_EQ(0, m.simulation(root,b1,p1,p2));
}

TEST(MCTS, simulation_2) {
    MCTS m;
    node *root = (node*) malloc(1 * sizeof(node));
    Board b1(4);
    Player p1(4, "user1", 1);
    ...
    ASSERT_EQ(-1, m.simulation(root,b1,p1,p2));
}

TEST(MCTS, simulation_3) {
    MCTS m;
    node *root = (node*) malloc(1 * sizeof(node));
    Board b1(4);
    Player p1(4, "user1", 1);
...
TEST(MCTS, update_1) {
    ...
    root->parent = nullptr;
    m.update(root, 1);
    ASSERT_EQ(1, root->totalCount);
    ASSERT_EQ(1, root->WinCount);
    ASSERT_EQ(log(1), root->logTotalCount);
...
TEST(util_func, query_1) {
    ...
    ASSERT_EQ("Input(row, col, weight): Out of boundary! Please choose another position.\nInput(row, col, weight): ", out_text);
    ASSERT_EQ(0, a.row);
    ASSERT_EQ(0, a.col);
    ASSERT_EQ(p1.id, a.id);
    ASSERT_EQ(1, a.index);
    ASSERT_EQ(3, a.card);
}

TEST(util_func, query_2) {
    MCTS m;
    node *root = nullptr;
    Board b1(4);
    Player p1(4, "user1", 1);
    ...
    std::cout.rdbuf(old_out);
    std::cin.rdbuf(old_in);
    ASSERT_EQ("Input(row, col, weight): You don't have this card: 3\nPlease choose another card.\nInput(row, col, weight): ", out_text);
    ASSERT_EQ(0, a.row);
    ASSERT_EQ(0, a.col);
    ASSERT_EQ(p1.id, a.id);
    ASSERT_EQ(0, a.index);
...
TEST(util_func, query_3) {
    ...
    std::cout.rdbuf(old_out);
    std::cin.rdbuf(old_in);
    ASSERT_EQ("Input(row, col, weight): Position can not be reused. Please choose another position\nInput(row, col, weight): ", out_text);
    ASSERT_EQ(1, a.row);
    ASSERT_EQ(1, a.col);
    ASSERT_EQ(p1.id, a.id);
    ASSERT_EQ(4, a.index);
